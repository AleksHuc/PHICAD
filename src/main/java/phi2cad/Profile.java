...
import latbirchi.CFTree;
import latbirchi.SplitChangeDifference;
...
public class Profile {
     ...
     */
    public Profile(HashMap<Integer,long[]> minMaxValues, int[] selectedValues, ArrayList<String> selectedValuesProcessing, int selectedKey, int maxChildren, double threshold, int pointLength,
                   double delta, double clearThreshold, int checkStep, int maxBins, int sizeOfBin, double thresholdT,
                   double timestampRateMax, DateTimeFormatter[] formats, boolean printOut, double lambda, int distanceFunction, double normalClusterThreshold, String name) {
...
//            put("bwd", new BIRCH_F(maxChildren, maxChildren, threshold, pointLength, delta, checkStep, maxBins, sizeOfBin, clearThreshold, thresholdT, timestampRateMax, lambda));
            put("fwd", new CFTree(maxChildren, distanceFunction, threshold, 200, lambda, delta, checkStep, maxBins, sizeOfBin, normalClusterThreshold, name + "_fwd"));
            put("bwd", new CFTree(maxChildren, distanceFunction, threshold, 200, lambda, delta, checkStep, maxBins, sizeOfBin, normalClusterThreshold, name + "_bwd"));
        }};
    }
     ...
     */
    ProfileUpdate updateProfile(FlowMessage flowMessage) {
...
//        }
        ...
        double[] detections = new double[]{adwinNormalized, adwinChange, interNormalized, intraNormalized, cluster};

        return new ProfileUpdate(this.clustering.get(flowDirection).getCopiedLeafArray(), timestamp, listToArray(clusteringVectorNormalized), this.clustering.get(flowDirection).getCentroid(), detections);
    }
